...
<%
    ...
    if(returncodeObj != null){
    	...
    	if(returncode == 0 ){
			...
			%>
			<form id="form1" style="padding: 10px;margin-top:-10px;" method="post" action = "../UpdateUserInfoServlet?updateKubun=2&returncode=<%=returncode%>">

			<div class="group" >
			    <div class="row">
				    <label class="row-left" style="width:100%;color:#333333;">销售员ID为系统生成，无法进行修改</label>
			    </div>
				<div class="row">
					<label class="row-left">销售员ID</label>
					<div class="row-right input-box" data-input="clear">
						<input type="text" name="user_id" value="<%=userinfo.getSaleman_id() %>"
							data-rule-field="<%=userinfo.getSaleman_id() %>" id="user_id" readonly="readonly" />
				...
				<div class="row">
    				...
    				<div class="input-box margin8 nolrmargin">
	    				<input type="text" placeholder="微信号" name="wechat" value="<%=userinfo.getWeixin_id() %>"
							data-rule-field="微信号" data-rule="required wechat english maxlength:15" />
					</div>
				</div>
			</div>
			<div class="group" >
				<div class="row">
    				...
    				<div class="input-box margin8 nolrmargin">
	    				<input type="text" placeholder="真实姓名" name="user_name" id="user_name" value="<%=userinfo.getSaleman_name() %>"
                    ...
                    <%
                        ...
                        if(null != userinfo.getSaleman_birthday()){
                        	 userBirthday = userinfo.getSaleman_birthday().toString().substring(0, 10);
                        }
                    %>
                <div class="row">
				...
				<div class="row">
    				...
    				<div class="input-box margin8 nolrmargin">
	    				<input type="text" placeholder="手机号码" name="telnumber" value="<%=userinfo.getSaleman_tel_num() %>"
							data-rule-field="手机号码" data-rule="phone maxlength:11" />
					</div>
				</div>
				<div class="row">
				    <label class="row-left">收货地址</label>
    				<div class="input-box margin8 nolrmargin">
	    				<input type="text" placeholder="收货地址" name="address" value="<%=userinfo.getSaleman_addr() %>"
							data-rule-field="收货地址" data-rule="maxlength:100" />
					</div>
				</div>
				<div class="row">
				    <label class="row-left">邮政编码</label>
    				<div class="input-box margin8 nolrmargin">
	    				<input type="text" placeholder="邮政编码" name="postNumber" value="<%=userinfo.getSaleman_post() %>"
							data-rule-field="邮政编码" data-rule="number maxlength:6" />
					</div>
				</div>
			</div>
			<div class="group" >
				<div class="row">
    				...
    				<div class="input-box margin8 nolrmargin">
	    				<input type="text" placeholder="支付宝账号" name="alipay_id" value="<%=userinfo.getAlipay_id() %>"
							data-rule-field="支付宝账号" data-rule="english maxlength:25" />
					</div>
				</div>
				<div class="row">
				    <label class="row-left">银行卡开户行</label>
    				<div class="input-box margin8 nolrmargin">
	    				<input type="text" placeholder="银行卡开户行" name="bankName" value="<%=userinfo.getCard_bank() %>"
							data-rule-field="银行卡开户行" data-rule="chinese maxlength:50" />
					</div>
				</div>
				<div class="row">
				    <label class="row-left">银行卡开户名</label>
    				<div class="input-box margin8 nolrmargin">
	    				<input type="text" placeholder="银行卡开户名" name="bankCreateName" value="<%=userinfo.getCard_name() %>"
							data-rule-field="银行卡开户名" data-rule="chinese maxlength:50" />
					</div>
				</div>
				<div class="row">
				    <label class="row-left">银行卡号</label>
    				<div class="input-box margin8 nolrmargin">
	    				<input type="text" placeholder="银行卡号" name="bankCardNo" value="<%=userinfo.getCard_number() %>"
							data-rule-field="银行卡号" data-rule="number 20" />
				    </div>
			   </div>
		    </div>
			<div class="group" >
            ...
            <div class="row">
                ...
                <%if(userinfo.getSaleman_sex() == null || "".equals(userinfo.getSaleman_sex()) || Integer.parseInt(userinfo.getSaleman_sex())==0){ %>
                ...
                <%}else if(Integer.parseInt(userinfo.getSaleman_sex())==1){ %>
                    <label><input type="radio" value="1" name="sex" checked/><span>男</span></label>
                    <label style="margin-left:12px;"><input type="radio" value="2" name="sex"/><span>女</span></label>
                <%}else if(Integer.parseInt(userinfo.getSaleman_sex())==2){ %>
                    <label><input type="radio" value="1" name="sex" /><span>男</span></label>
		    ...
		    <button class="button submit block" type="submit">确定修改</button>
			<%
			        }
			    }
			%>
